...
import { getAccessToken, isExpiredAccessToken, handleRefreshToken } from 'api'

export const getTokens: any = createAsyncThunk('app/auth', async (code: string) => {
  try {
...
export const handleAuth: any = createAsyncThunk('app/auth', async () => {
  try {
    ...
    if (!await isExpiredAccessToken(accessToken) && refreshToken) {
